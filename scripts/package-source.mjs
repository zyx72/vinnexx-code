...
const archiveName = "vinnexxcode0.3.zip";
const archive = resolve(release, archiveName);
const checksum = `${archive}.sha256`;
const temp = await mkdtemp(join(tmpdir(), "vinnexx-source-"));
const stageName = "vinnexxcode0.3";
const stage = join(temp, stageName);
const excluded = new Set(["node_modules", ".git", ".vercel", "dist", "build", ".next", ".cache", "coverage", "release"]);
...
function filter(source) {
  ...
  if (/\.(log|pem|key|p12|pfx)$/i.test(name)) return false;
  if (/^(credentials|secrets).*\.json$/i.test(name)) return false;
  return true;
}
