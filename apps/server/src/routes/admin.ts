...
const configSchema = z.object({
  ...
  strummerPrompt: z.string().min(10).max(20_000),
  unitedPrompt: z.string().min(10).max(20_000)
});
...
export async function adminRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/admin/users", async (_request, reply) => {
    ...
    return ok(reply, users.map((user) => ({
  ...
  app.put("/admin/users/plan", { preHandler: [requireTrustedOrigin] }, async (request, reply) => {
  ...
  app.get("/admin/system-config", async (_request, reply) => {
    ...
    return ok(reply, {
      ...
      strummerPrompt: config.strummerPrompt,
      unitedPrompt: config.unitedPrompt,
      updatedAt: config.updatedAt
    });
  });

  app.put("/admin/system-config", { preHandler: [requireTrustedOrigin] }, async (request, reply) => {
    const body = configSchema.parse(request.body);
    await collections().systemConfig.updateOne(
      { _id: "ai" },
      { $set: { ...body, promptVersion: 3, updatedAt: new Date() } },
      { upsert: true }
    );
    return ok(reply, { updated: true });
  });
